...
def clean_text(text):
    # FIX: corrected double-escaped backslashes in regex patterns
    text = str(text).lower()
...
<div class="container">
    ...
    <div class="upload-box">
        ...
        <textarea id="textInput" placeholder="Paste AI or Human text here..."></textarea>
...
async function uploadDataset() {
    ...
    try {
        ...
        statusEl.className = data.message.includes("Error") || data.message.includes("must") || data.message.includes("small")
            ? "status error"
            : "status success";
...
// ─── Voice Typing ─────────────────────────────────────────────────────────────
// FIX: removed the dangling stray `}` and cleaned up the onclick binding logic
...
function startVoiceTyping() {
    ...
    let finalTranscript = document.getElementById("textInput").value;
...
def train():
    ...
    global model1, model2, vectorizer
    ...
    try:
        ...
        # FIX: corrected double-escaped backslashes in RTF regex
        ...
        if len(data.columns) < 2:
            return jsonify({"message": "Dataset must contain at least 2 columns"})
        ...
        if dataset_size < 10:
            return jsonify({"message": "Dataset too small. Upload at least 10 rows."})

        # Adaptive TF-IDF settings based on dataset size
        if dataset_size < 100:
            max_features = 3000
            ngram        = (1, 1)
            max_df_val   = 1.0
        ...
        train_extra = np.vstack([extract_advanced_features(t)[0] for t in X_train])
        test_extra  = np.vstack([extract_advanced_features(t)[0] for t in X_test])

        X_train_final = hstack([X_train_vec, train_extra])
        X_test_final  = hstack([X_test_vec,  test_extra])

        model1 = LinearSVC(max_iter=2000)
        ...
        # FIX: proper ensemble — use model2 vote to break ties instead of ignoring it
        final_pred = []
        for p1, p2 in zip(pred1, pred2):
            if p1 == p2:
                final_pred.append(p1)
            else:
                # On disagreement prefer the RandomForest (usually more calibrated)
                final_pred.append(p2)

        accuracy = round(accuracy_score(y_test, final_pred) * 100, 2)

        return jsonify({
            "message": f"Advanced Model Trained Successfully | Accuracy: {accuracy}%"
        })
...
def predict():
    ...
    if model1 is None:
        return jsonify({"error": "Please Upload And Train Dataset First"})

    # FIX: guard against missing/empty text field
    ...
    extra_features  = extract_advanced_features(cleaned_text)
    final_input     = hstack([vectorized_text, extra_features])

    pred1 = model1.predict(final_input)[0]
    pred2 = model2.predict(final_input)[0]

    # FIX: proper ensemble tie-break (same logic as training)
    if pred1 == pred2:
        prediction = pred1
    else:
        prediction = pred2

    raw_score  = abs(model1.decision_function(final_input)[0])
    # FIX: added minimum floor so short texts don't show 0% confidence
    confidence = round(min(max(raw_score * 20, 10), 99), 2)

    # Heuristic override: low burstiness + AI keyword density → likely AI
    ai_patterns = [
        "overall", "moreover", "furthermore", "in conclusion",
        "therefore", "additionally", "however"
    ]
    pattern_hits = sum(1 for p in ai_patterns if p in text.lower())
    ...
    if prediction == 1:
        ai_score    = round(min(60 + confidence / 2, 99), 2)
        human_score = round(100 - ai_score, 2)
        result      = "AI Generated Text"
    else:
        human_score = round(min(60 + confidence / 2, 99), 2)
